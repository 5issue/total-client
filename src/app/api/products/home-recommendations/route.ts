...
/**
 * 홈 화면 진열 데이터 조회 — `useHomeRecommendations`가 호출. 인증 불필요(`publicFetch`).
 * `src/app/api/products/[productId]/route.ts`와 동일한 패턴 — upstream 실패는 원인을
 * 가리지 않고 같은 메시지로 내린다(구현 세부 노출 방지).
 */
const UPSTREAM_FAILURE_MESSAGE = '홈 화면 정보를 불러오지 못했습니다.';

/** upstream 응답 헤더 대기 상한 — 없으면 Node 24 기본 fetch(Undici) 타임아웃(300초) 동안
 * Route Handler와 클라이언트 요청이 pending 상태로 남는다. */
const UPSTREAM_TIMEOUT_MS = 5000;

export async function GET() {
  let raw;
  try {
    const springRes = await fetch(`${env.API_INTERNAL_URL}/api/v1/products/home-recommendations`, {
      headers: { 'Content-Type': 'application/json' },
      signal: AbortSignal.timeout(UPSTREAM_TIMEOUT_MS),
    });
    raw = SpringEnvelopeSchema(HomeRecommendationsResponseSchema).parse(await springRes.json());
  } catch {
    // 연결 실패·비 JSON 응답·스키마 불일치가 전부 여기로 온다. 그대로 두면 Next 의
    // 프레임워크 오류 응답(봉투 없는 500)이 나가 `publicFetch` 가 `ApiEnvelope` 를
    // 못 받고, 화면은 에러 대신 로딩에 머문다.
