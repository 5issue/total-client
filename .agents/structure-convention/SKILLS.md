...
> **⭐ 상태 커버리지 = 디자인 핸드오프 목록이 전부다.**
> 상태 분기(code-style §2-2 step 4)·스토리(git-convention S2-6)·예외 처리·PR 체크리스트(git-convention §3)는 모두 **디자인팀이 _그 컴포넌트_ 에 대해 넘긴 상태값 목록** 만 커버한다.
> `Default/Loading/Empty/Error/Disabled` 같은 나열은 예시이지 5종 체크리스트가 아니다.
