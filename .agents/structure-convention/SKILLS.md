...
### 6-1. 핸드오프 · 변경 공유 (2026-09-01 결정)

디자인팀 일정상 전체 스펙 문서를 먼저 받는 방식(1안)은 화면 디자인·프로토타입·UT 를 밀리게 하므로,
**"디자인팀은 상태값, 프론트엔드는 스펙" 분담(2안)** 으로 간다.

| 구분       | 담당                                                                                                                                                                                                                                                      |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 디자인팀   | 컴포넌트·화면의 **상태값 목록**과 **인터랙션 조건**. 화면은 Figma description 에 설명 첨부. 상태값은 컴포넌트마다 다르다 — `default / hover / focus / disabled / loading / empty / error / selected` 는 **자주 나오는 예시일 뿐 범용 필수 목록이 아니다** |
| 프론트엔드 | 수치·간격·컬러·타이포·radius·모션 등 **구체 스펙을 Figma 에서 직접 확정** — MCP `get_design_context` / `get_variable_defs` 로 추출, 값 1:1, 임의값 금지 (code-style §2-2·§6-1)                                                                            |

- **변경 공유**: 프레임·컴포넌트가 수정되면 **해당 노드의 Figma 코멘트**로 "무엇이 어떻게 바뀌었는지" 1~2줄. (Slack/Discord 스레드 아님)
- **정리·구현 우선순위**: ① 스펙(수치·간격·컬러·상태값) → ② 예외 처리(그 컴포넌트에 해당하는 상태만 — 로딩·에러·빈 상태·스켈레톤 등) → ③ 사용 예시. 재사용 컴포넌트가 많아 정확한 수치·상태값이 최우선, 그다음 예외 케이스로 구현 누락을 줄인다.

> **⭐ 상태 커버리지 = `Default`(항상) + 디자인 핸드오프 목록.**
> 상태 분기(code-style §2-2 step 4)·스토리(git-convention S2-6)·예외 처리·PR 체크리스트(git-convention §3)는 모두 **기본 렌더(`Default`) + 디자인팀이 _그 컴포넌트_ 에 대해 넘긴 상태값 목록** 만 커버한다. (`Default` 는 상태라기보다 baseline 이라 핸드오프에 없어도 항상 1개.)
> `Loading/Empty/Error/Disabled` 같은 나열은 예시이지 필수 체크리스트가 아니다.
> **설계상 없는 상태를 위해 스토리·prop·API 를 새로 만들지 않는다** — 예:
>
> - `Calendar` 에 `Loading` 상태가 없다 (표시 데이터가 동기)
> - 옵션이 정적인 select-only `Dropdown` 에 `Error` 상태가 없다 (로딩/에러는 상위 쿼리 계층 몫 — code-style §3-1)
> - 순수 표현용 `Accordion`·`Tooltip` 에 `Empty`/`Error` 가 없다
>
> 리뷰(사람·자동 도구 모두)는 "없는 상태가 빠졌다"를 지적하지 않는다. "핸드오프 목록에 있는 상태가 빠졌다"만 지적한다.

- 이 결정으로 화면 디자인 전달은 2026-09-01 부터, 컴포넌트 스펙 정리본은 2026-09-04 이후 가능.

### 6-2. 협의 필요 · 보류 항목

아래는 **"무엇을 만들지"(정책·범위)** 자체가 미정이라 디자인/기획 합의 전까지 임의 구현하지 않는다.
(§6-1 대로 시각 **스펙**(수치·컬러·타이포·radius·모션)은 FE 가 Figma 에서 확정하고 상태값·인터랙션 조건은 디자인팀 핸드오프를 따르되, 아래는 그 스펙 이전의 결정 사항)
